...
2. For some reason you can only add the new widgets defined by `SheeranXLib.msl` to the "Floating Track Strip" tab if you to try and add them via  `Display -> Edit Layouts...`. To get round this you have to manually edit the `uiconfig.xml` file.
	i. Go to Möbius' Configuration Root folder (which you can find by selecting `Help -> About`) and open the `uiconfig.xml` file in a text editor.  Look for the section of text that looks a bit like this:
	```xml
  ...
  <Layout name="Default">
	...
	ii. From the "Floating Track Strip" tab, drag MuteModeLabel, MuteModeLight, fmodeLabel, fmodeLight, fmenuALabel, fmenuBLabel and fmenuTapLabel from the right panel to the left panel and then click on Save.
